...
void MetaD::computeReweightingFactor()
{
  ...
  for (Grid::index_t t=rank; t<BiasGrid_->getSize(); t+=stride) {
    ...
    if(t==rank)
    {
		Z_0 = minusBetaF*val-big_number;
		Z_V = minusBetaFplusV*val-big_number;
	}
	else
	{
		exp_added(Z_0,minusBetaF*val-big_number);
		exp_added(Z_V,minusBetaFplusV*val-big_number);
	}
  }
  //~ if (stride>1) {
    //~ comm.Sum(Z_0);
  ...
  if(stride>1)
  {
    ...
    if(comm.Get_rank()==0)
    {
      multi_sim_comm.Allgather(Z_0,all_Z_0);
      multi_sim_comm.Allgather(Z_V,all_Z_V);
    }
    comm.Bcast(all_Z_0,0);
    comm.Bcast(all_Z_V,0);
  
    Z_0 = all_Z_0[0];
    Z_V = all_Z_V[0];
    for(unsigned i=1;i<stride;++i)
    {
		exp_added(Z_0,all_Z_0[i]);
		exp_added(Z_V,all_Z_V[i]);
	}
  }
