...
void ReweightITS::registerKeywords(Keywords& keys ){
  ...
  keys.add("compulsory","ARG","the biases that must be taken into account when reweighting. If you want to reweight the system at other temperatures, please set the last argument as the energy of the system"); 
  //~ keys.add("optional","ENERGY","*.energy","the energy of the system. It will be used when reweight to other temperatures");
  //~ keys.add("optional","FB_FILE","using fb file to calculate reweighting factor"); 
  keys.add("optional","RWTEMP","the reweighting temperature.  If it is not assigned, it will be equal to the system temperatures");
  keys.add("optional","SHIFT_CONST","a constant value to shift the bias potential when they are too large or too small");
}
...
shift_const(0)
{
	...
	if(rw_other==true&&getNumberOfArguments()<=1)
		error("you want to reweight at other temperature but only one argument value was setup. Please fill the ARG value as the bias and energy, and the energy value should be the last one");
}

double ReweightITS::getLogWeight() const {
	if(rw_other)
	{
		double energy=getArgument(getNumberOfArguments()-1);
		double bias=0.0; for(unsigned i=0;i<getNumberOfArguments()-1;++i) bias+=getArgument(i);
		return beta0*bias + (beta0-rwbeta)*energy + shift_const;
	}
	else
	{
		double bias=0.0; for(unsigned i=0;i<getNumberOfArguments();++i) bias+=getArgument(i);
		return bias / simtemp + shift_const;
	}
}
